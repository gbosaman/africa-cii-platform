...
The AdSense loader is rendered in the root layout, so it appears inside `<head>` on every page —
verified at 119 of 119 prerendered pages. The tag emitted is exactly the one AdSense supplies:
...
**Why a plain `<script>` rather than `next/script`.** Both `afterInteractive` and
`beforeInteractive` emit a `<link rel="preload">` plus a deferred injector rather than the literal
tag, which is harder for AdSense verification to see. A plain element produces the exact markup
Google asks to be pasted.

**It is gated to production.** `process.env.NODE_ENV === "production"` keeps the loader out of
localhost and dev builds. Ad impressions generated by the developer refreshing their own pages are
precisely what Google's invalid-traffic enforcement exists to catch, and the cost of tripping it is
the account, not the page. Vercel builds with `NODE_ENV=production`, so the live site is unaffected.
